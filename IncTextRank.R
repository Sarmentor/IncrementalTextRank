...
PR <- function(d,threshold,i.graph, text_nodes,nodes_num,nodes_rank){
  k <- 0                                  # iterations
...
incrementalPR <- function(d,threshold,i.graph,text_nodes, affected_nodes, nodes_rank){
  #browser()
  k <- 0                                  # iterations
  convergence_reached <- FALSE
...
updatePR <- function(changed.nodes.vector, nodes_num, new.nodes_num){
  ...
  incrementalPR(d,threshold,i.graph, as.character(V(i.graph)$name), affected.nodes, nodes_rank)
  
}

post.proc <- function(words, text_nodes, nodes_num, nodes_rank, selected_words,words_with_punctuation){
...
inc.post.proc <- function(words, text_nodes, nodes_num, nodes_rank, selected_words,words_with_punctuation, affected.nodes){
...
main <- function(stream.doc, iter){
  ...
  if(iter == 1){
    ...
    i.graph <- igraph.from.graphNEL(text_graph, name = TRUE, weight = TRUE,
                                    unlist.attrs = TRUE)
    PR(d,threshold,i.graph, text_nodes,nodes_num,nodes_rank)
    ### Post-Processing ###
    
    new.keys <- post.proc(words, text_nodes, nodes_num, nodes_rank, selected_words,words_with_punctuation)
    #print(new.keys)
    ...
    new.keys <- inc.post.proc(words, text_nodes, nodes_num, nodes_rank, selected_words,words_with_punctuation, changed.nodes.vector)
    #print(new.keys)
    ### Top-K Keywords Update ###
    final.key.list <- topk(new.keys)
  }
...
go <- function(){
  ...
  save(list=c("time.main2",paste("iter.",1:6,".key.list",sep=""),paste("nnodes.iter.",1:6,sep=""),paste("nwords.iter.",1:6,sep="")),file="IncTextRankResults15-5-2.RData")
